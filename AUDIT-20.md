...
**Nachtrag (2026-08-20): ganz entfernt.** Die „Zurückbauen"-Frage wurde
entschieden — die vier Felder (`alpha`/`blendModeName`/`isVisible`/`label`) und die
gesamte Maschinerie (`WallpaperBlendMode`-Enum, Mapper, `AVAILABLE_BLEND_MODES`,
die 12 `blend_mode_*`-Strings, die `onSetLayer*`-Setter, `nextFreeAutoLabel`) sind
komplett raus (`refactor/drop-dormant-layer-props`). Kein Backup-Bruch: der Import
ignoriert unbekannte Keys (`ignoreUnknownKeys` + read-by-known-key-Parser), alte
Backups restaurieren weiter. Kein Verhaltenswechsel (Felder waren immer Default).
Details: `ACCEPTED_LIMITATIONS.md` §5 (Tombstone) + §1 (Classifier-Gate reconciled).
