...
| 22 | `SettingsActivity` auf Edge-to-Edge + `MaterialToolbar` modernisieren | **offen** (2026-09-12, aufgefallen bei der Nyx-Settings-Angleichung) — `SettingsActivity` ist die **einzige** Kolibri-Activity ohne `WindowCompat.setDecorFitsSystemWindows(window, false)` (MainActivity/CustomNames/HiddenApps/Onboarding/SwipeActions haben es) und nutzt noch die Decor-ActionBar. Funktioniert nur, weil sie bewusst nicht-edge-to-edge bleibt — ein Alt-Stand. Nyx macht es inzwischen moderner (expliziter `MaterialToolbar` im Layout + `fitsSystemWindows`, edge-to-edge, kein Decor-ActionBar-Sonderfall). Kolibri nachziehen: gleiche Toolbar-Variante, damit alle Activities edge-to-edge konsistent sind. Anker: `SettingsActivity`, `activity_settings.xml`, `SettingsFragment`. | klein-mittel |
