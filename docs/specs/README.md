...
Living design documents for the major subsystems. These were the blueprints for
the big rewrites; now that those shipped, they are the authoritative **rationale
and invariants** the production code delegates to — a code comment saying
"`DATASTORE_READ_SPEC` Belang A" resolves here. They are cited by **bare name**
(not path), so this folder can be reorganised without touching code.

Not blueprints-to-discard: 14 of them are actively referenced from `.kt` (up to
24 references for `DATASTORE_READ_SPEC`). Rejected / superseded design
alternatives live in [`../history/`](../history/), not here.
