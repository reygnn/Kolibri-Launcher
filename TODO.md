...
| 22 | `SettingsActivity` auf Edge-to-Edge + `MaterialToolbar` modernisieren | **erledigt 2026-09-12** — expliziter `MaterialToolbar` im `activity_settings.xml` (über `settings_container`-FrameLayout, `fitsSystemWindows` + `colorSurface`), Activity auf `WindowCompat.setDecorFitsSystemWindows(window, false)` + `setSupportActionBar(toolbar)` umgestellt (ViewBinding, crash-safe Struktur bleibt), leeres `SettingsTheme` entfernt und die Activity im Manifest auf `@style/AppTheme` gezogen — damit ist sie nicht mehr die einzige Decor-ActionBar-Activity, alle sind edge-to-edge konsistent. Auf Pixel 9a verifiziert. Ursprünglich aufgefallen bei der Nyx-Settings-Angleichung (Nyx war die Toolbar-Referenz). | klein-mittel |
