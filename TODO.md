...
## 3. XML-Bullshit fail-fast: Lint scharfstellen

Klassische Wunde: ein neuer String in `values/strings.xml` wird in
`values-de/strings.xml` vergessen. Compiler nickt (XML-Syntax stimmt),
Build durchläuft, Release fliegt raus, deutsches Phone öffnet das
betroffene Fragment → `Resources.NotFoundException` zur Laufzeit.
Schon einmal in einem Hotfix passiert.

### Aktueller Stand (Stand 2026-04-30)

- [x] ~~`app/build.gradle.kts` enthält **keinen** `lint { ... }`-Block~~
  — bestätigt; Lint läuft mit Android-Defaults
- [x] ~~`lint.xml` im Repo existiert **nicht**~~ — bestätigt
- [x] ~~CI (`.github/workflows/android.yml`) ruft Lint **nicht**~~ —
  bestätigt; Workflow startet nur `testDebugUnitTest`,
  `jacocoTestReport`, `assembleDebug`

Ergebnis: das XML-Bullshit-Szenario ist heute strukturell offen.

### Aufgaben

- [ ] **`lint { ... }`-Block in `app/build.gradle.kts` ergänzen**
  - `abortOnError = true` (auch für `assembleRelease`)
  - `warningsAsErrors = false` (nicht jede Lint-Warning soll bremsen —
    nur die wirklich gefährlichen)
  - Severity-Overrides für die XML-Killer:
    - `error("MissingTranslation")` — fehlende `values-de/`-Strings
    - `error("ExtraTranslation")` — verwaiste DE-Strings ohne EN-Pendant
    - `error("MissingDefaultResource")` — Resource nur in einer Variante
  - Optional zusätzlich, weniger schmerzhaft: `error("InvalidResource\
    Folder")`, `error("InconsistentArrays")`, `error("IconDuplicates")`
  - `checkReleaseBuilds = true`

- [ ] **CI um Lint-Phase erweitern**
  - In `.github/workflows/android.yml` zwischen Test und Build:
    `./gradlew lintDebug --stacktrace`
  - Bricht der Workflow bei Lint-Errors ab → kein Release mit kaputtem
    XML.
  - Optional: `lint-results.html` als CI-Artefakt hochladen, damit man
    bei rotem Build direkt reinschauen kann.

- [ ] **Sanity-Check: einmal manuell `./gradlew lint` laufen lassen**
  - Bevor `abortOnError = true` aktiviert wird, einmal lokal sehen,
    welche Lint-Issues heute schon im Tree liegen.
  - Falls viel: `lint-baseline.xml` einmalig generieren, damit nur
    *neue* Issues blockieren — alte sammeln sich nicht im PR auf.
