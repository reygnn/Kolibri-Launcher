 ...
 * [FakeDataStore]. The impl has a plain `@Inject` constructor (no
 * `shareIn` layer), so it is constructed directly.
 */
class CrashReportConsentRepositoryImplContractTest : CrashReportConsentRepositoryContract() {
    ...
    override fun createRepository(): CrashReportConsentRepository {
        return CrashReportConsentRepositoryImpl(dataStore = FakeDataStore())
    }
}
