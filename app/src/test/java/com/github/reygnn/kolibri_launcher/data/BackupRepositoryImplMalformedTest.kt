...
/**
 * Eine "fiese" Test-Suite, die den BackupRepositoryImpl mit syntaktisch falschem,
 ...
 */
class BackupRepositoryImplMalformedTest {
    ...
    fun `importFromJson handles JSON with trailing garbage`() = runTest(testDispatcher) {
        ...
        // In BackupRepositoryImpl fängt tryStrictParsing Exceptions.
    ...
    fun `importFromJson fails when required 'settings' object is missing`() = runTest(testDispatcher) {
        ...
        // BackupRepositoryImpl fängt das und gibt null zurück.
        assertThat(result).isEqualTo(ImportResult.InvalidFormat)
    }
    ...
    fun `importFromJson handles empty string`() = runTest(testDispatcher) {
        ...
        // Hier erwarten wir InvalidFormat oder Error, abhängig davon wie BackupRepositoryImpl "blank" prüft.
