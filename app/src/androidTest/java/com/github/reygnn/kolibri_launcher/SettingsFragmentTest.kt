...
class SettingsFragmentTest : BaseAndroidTest() {
    ...
    @Test
    fun textReadabilityPreference_showsListDialog() = testCoroutineRule.runTestAndLaunchUI {
        launchFragmentInHiltContainer<SettingsFragment>()
        onView(withText(R.string.text_readability_title)).perform(click())

        onView(withText(R.string.text_readability_title))
            .inRoot(isDialog())
