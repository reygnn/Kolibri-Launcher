package com.github.reygnn.launcher.core.crashreporting.ingestion

import java.util.concurrent.CancellationException

/**
 * Builds the throwable that `AcraTree` forwards to ACRA, folding the Timber log
 * context (priority, tag, message) INTO the reported throwable instead of
...
/**
 * Carrier exception whose message carries the Timber log context. Public (not
 * `internal`) because, now that this type lives in the shared `:core` module,
 * the `ReportCarrierTest` in `:kolibri:domain` references it across the module
 * boundary — where Kotlin `internal` is not visible.
 */
